...
@OnlyIn(Dist.CLIENT)
public class GhzjtvzhzvModel<T extends Entity> extends SegmentedModel<T> implements IHasHead, IHeadToggle {
   ...
   public GhzjtvzhzvModel(float scale) {
      this(scale, 64, 64);
   }

   public GhzjtvzhzvModel(float p_i51059_1_, int p_i51059_2_, int p_i51059_3_) {
      float f = 0.5F;
   ...
   public void setRotationAngles(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      boolean flag = false;
      if (entityIn instanceof AbstractGhzjtvzhzvEntity) {
         flag = ((AbstractGhzjtvzhzvEntity)entityIn).getShakeHeadTicks() > 0;
      }
